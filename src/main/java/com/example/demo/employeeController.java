        ...
        Search.textProperty().addListener((observable, oldValue, newValue) -> {
            filteredList.setPredicate(employee -> {
                ...
                return String.valueOf(employee.getId()).contains(val) ||
                        employee.getName().toLowerCase().contains(val) || String.valueOf(employee.getRole_Id()).contains(val) ||
                        String.valueOf(employee.getSalary()).contains(val) || employee.getUser_Name().toLowerCase().contains(val)
    ...
    public void fillTable() {
        try {
            ...
            while (res.next()) {
                ...
                emp.setUser_Name(res.getString(5));
                emp.setSalary(res.getDouble(8));
                emp.setRole_Id(res.getInt(9));
                Empdata.add(emp);
            }
